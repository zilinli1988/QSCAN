#' Mean SCAN (M-SCAN) statistic based procedure
#'
#' The \code{M_SCAN} function takes in genotype, phenotype and covariates and detect the association between a
...
#' @return \code{SCAN_top1}:  A vector of length 4 which summarized the top 1 region detected by M-SCAN.
...
M_SCAN <- function(genotype,phenotype,X,family,Lmax,Lmin,steplength=1,times=2000,alpha=0.05,f=0)
{
	...
	for(i in 1:folds)
	{
		...
		set.seed(19880615)
		threstemp <- SCAN_Thres_M(genotypesub,X,working,sigma,fam,times,Lmax,Lmin,steplength,weightssub)
		...
		restemp <- SCAN_Search_M(genotypesub,X,working,sigma,fam,phenotype,mu0,th0,Lmax,Lmin,steplength,weightssub,begid,f)
